...
children <-
  ...
  ungroup() %>% 
  select(
    serial, momloc_c = momloc, twin1, twin2, children_hh, age_c = age, 
    qage_c = qage, qsex_c = qsex, qrelate_c = qrelate, qbirthmo_c = qbirthmo
  )
...
```{r}
sample1 <-
  left_join(mothers, children, by = c("serial", "pernum" = "momloc_c")) %>%
  filter(
...
# Paper results:   662,204   535,887   465,595
```

```{r}
sample2 <- 
  sample1 %>% 
  mutate(children_ever = chborn - 1) %>% 
  filter(children_ever == children_hh, age_c < 18, is.na(twin1) | twin2 == 0)

sample1 %>% select(serial, pernum, chborn, children_hh, age_c) %>% View()
nrow(mothers2)
