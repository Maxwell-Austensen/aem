...
rd_plot_starter <- function(y) {
  clark %>% 
    ...
    xlab("Percentage vote in favour of the GM status") +
    theme_minimal()
}
...
With all the different ranges the effect estimates remained fairly close, and all were statistically significant at the 1% level using robust standard errors, except for the smallest range (40, 60) where hte estiamte was significant at the 5% level.
...
```{r}
clark %>% 
  ggplot(aes(vote, passrate2, group = factor(win))) +
  geom_smooth(
    method = "loess", 
    color = "black", 
    level = .95
  ...
  points_and_format() +
  labs( # Overwrite the labels (set above in helper function) for passrate2
      y = "Pass rate of pupils two years after vote",
      x = "Percentage vote in favour of the GM status"
    ) +
  ggtitle("Loess smoother, applied separately over win and lose voteshares")
```
...
The results using `passrate2` differ substantially from those using `dpass`, most notably in that they are not statistically significant using any of the four sample ranges. The positive and significant results for the change from baseline and the lack of significant results for the post intervention outcomes suggests that schools on either side of the win threshold differed in their outcome variable at baseline. This in theory would seem to violate the requirement for regression dicontinuity design that the schools on either side of the threshold do not differ significantly on any pre-treatment variables relevant to the post-treatment outcome. However, in practice, Clark suggests the fact that the pass rates are uniformly higher reflects "the general increase in pass rates seen over the 1990s."

```{r}
rd_df <- function(dep_var) {
  f <- paste0(dep_var, " ~ vote")
  
  rd_obj <- clark %>% RDestimate(data = ., formula = f, cutpoint = 50)
...
The results from the sharp RD local linear regressions using `dpass` as the outcome differ from the first specification using the sample 15-85 and only athe binary `win` indicator, in that the estimate is larger, but most importatantly in that it is not signifciant. The results using `passrate2` as the dependant variable also differ when using the local linear regression: the estimates are still not significant, but the effect estimate is negative where before it was positive. the  Imbens-Kalyanaraman optimal bandwidths used for both outcome variables is small and thus includes only 105 and 157 observations for `dpass` and `passrate2`, respectively which limits the percision of the estimate considerably compared to the first specifications that include 524 observations in the sample voteshare range (15, 85).

The fact that the effect estimates are not significant in the local linear model raises some doubts about the robustness of the results from the polynomial specifications used by Clark. However, looking at the plot below, it does not appear that the third-order polynomial used by Clark is exagerating the disconinutiy at the threshold
...
clark %>% 
  ...
  points_and_format() +
  labs( # Overwrite the labels (set above in helper function) for passrate0
      y = "Pass rate of pupils in the year immediately prior to the vote",
      x = "Percentage vote in favour of the GM status"
    ) +
  ggtitle("Loess smoother, applied separately over win and lose voteshares")
```
...
m2 <- clark %>% model_prep(passrate0, c(25, 75))
m3 <- clark %>% model_prep(passrate0, c(33, 66))
...
stargazer_html(
  ...
  keep.stat = c("n", "rsq"),
  omit = "Constant",
  add.lines = list(c("Sample Vote Range", m1[[3]], m2[[3]], m3[[3]]))
)
```
...
```{r}
bandwidths <- seq(0.75, 1.25, 0.10) %>% map_dbl(~ .x * rd_dpass$bw[[1]])
